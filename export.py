...
def _encabezado_individual(ws, empleado: dict, mes: int, anio: int,
    ...
    campos = [
        ...
        ("Subespecialidad:",    empleado.get("Subespecialidad", "")),
        ("Alta Especialidad:",  empleado.get("Alta_Especialidad", "")),
        ("Grado:",              empleado.get("Grado", "")),
        ("Periodo Ingreso:",    empleado.get("Periodo_Ingreso", "")),
...
def exportar_reporte_empleado(df_empleado: pd.DataFrame,
    ...
    for anio, mes in periodos:
        mask   = (fechas_dt.dt.year == anio) & (fechas_dt.dt.month == mes)
        df_mes = df_empleado[mask].copy()
        if df_mes.empty:
            continue

        dias_mes  = calendar.monthrange(anio, mes)[1]
        titulo_ws = f"{MESES_ES.get(mes, mes)} {anio}"
        ...
        if first:
            ...
            ws = wb.create_sheet(title=titulo_ws)

        ws.column_dimensions["A"].width = 14
        _encabezado_individual(ws, empleado, mes, anio, ruta_logo)
        fila_sig = _calendario_individual(ws, df_mes, mes, anio, fila_ini=13)
...
def exportar_todos(df_resultado: pd.DataFrame,
    ...
    for i, id_emp in enumerate(ids):
        df_emp = df_resultado[df_resultado["ID"] == id_emp].copy()
...
def exportar_maestro_consolidado(df_resultado: pd.DataFrame,
    ...
    for abrev, etiqueta, hex_color in LEYENDA_ITEMS:
        ...
        c_color.alignment = ALIN_C
        c_color.border    = BORDER
...
def _construir_hoja_resumen_general(ws, df: pd.DataFrame,
    ...
    c = ws.cell(row=1, column=1,
                 value=f"SGAM – Resumen General de Asistencia  |  {mes_str} {anio}")
    c.font      = _font(bold=True, size=13, color="1F3864")
    c.fill      = _fill("EBF3FB")
    c.alignment = ALIN_L
    ...
    for j, (h, w) in enumerate(zip(headers, anchos), start=1):
        c = ws.cell(row=FILA_H, column=j, value=h)
    ...
    for idx, est in enumerate(orden):
        ...
        cnt  = conteos[est]
        pct  = round(cnt / total_dias * 100, 1) if total_dias > 0 else 0.0
        info = COLOR_MAP.get(est, {"label": est, "hex": "FFFFFF"})

        datos = [info["label"], cnt, f"{pct}%", ""]
        for j, val in enumerate(datos, start=1):
            c = ws.cell(row=fila, column=j, value=val)
            c.font      = _font(size=9)
            c.alignment = ALIN_C
            c.border    = BORDER
    ...
    c = ws.cell(row=fila_tot, column=1, value="TOTAL DÍAS EVALUADOS")
    c.font = _font(bold=True, size=9, color="1F3864")
    c.fill = _fill("DEEAF1"); c.alignment = ALIN_L; c.border = BORDER
    c2 = ws.cell(row=fila_tot, column=3, value="100%")
    c2.font = _font(bold=True, size=9); c2.fill = _fill("DEEAF1")
    c2.alignment = ALIN_C; c2.border = BORDER